...
class APIWrapper:
    ...
    def __init__(self):
        self.response = None
        pet_data_file_path = 'C:\AutomationWithTsahi\APITesting\pet_data.json'
        config_file_path = 'C:\AutomationWithTsahi\APITesting\config_Pet_swagger.json'
        store_file_path = 'C:\AutomationWithTsahi\APITesting\store.json'
        user_file_path = r'C:\AutomationWithTsahi\APITesting\user_data.json'
    ...
    def api_put_request(self, endpoint, data=None):
        url = self.base_url + endpoint
        self.response = requests.put(url, json=data)  # Assuming JSON data for PUT request
        if self.response.ok:
            return self.response
        else:
            return self.response.status_code
